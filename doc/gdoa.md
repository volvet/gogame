...
### Adadelta
Adadelta是adagrad算法的扩展，  它寻求解决学习因子单调递减的问题.  跟adagrad算法需要求历史所有梯度的平方和不同， adadelta将球平方和的窗口限制为某个固定值$w$. 为了避免在训练过程中需要持续保存$w$个历史梯度值,  adadelta寻求一种更为高效的做法， 令$E_t$为阶段$t$的均值， 它仅被$\gamma$和前一阶段的均值$E_{t-1}$所决定($\gamma$类似momentum).
